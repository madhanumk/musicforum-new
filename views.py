...
from .models import  Event_Organizer_Highlight, Event_Performer, Reply, Skill_Category, User, Troupe, Event_Organizer, Gurukulam, Musician, Musician_Award, Musician_Highlight, Musician_Skill, Musician_Gurukulam, Gurukulam_Skill, Gurukulam_Award, Gurukulam_Testimonial, Lecture_Video, Course, Troupe_Highlight, Key_Performer, Location, Troupe_Skill, Post, UserFollowing, Post_Comment, Highlight, Blocked_User, Chat, Conversation, Skill_Level, Gurukulam_Teacher
from django.contrib.auth.views import LoginView
from django.contrib.auth import logout as user_logout
from functools import wraps
from itertools import chain
import ast
from event.models import Event, Event_Skill, Invitation, Interest, Message, Skill, Event_Category, Event_Type
...
def search_people(request):
    ...
    if page == "people":
        if people_type == "eventer":
            ...
            users = User.objects.filter(Q(last_name__icontains=name) | Q(first_name__icontains=name),groups__name="Musician").exclude(id=request.user.id).distinct()[:50]
        ...
        if people_type == "eventer":
            ...
            users = User.objects.filter(Q(last_name__icontains=name) | Q(first_name__icontains=name),groups__name="Musician").exclude(id=request.user.id).distinct()[:10]
        elif people_type == "troupe":
    ...
    if model_name == 'musician_gurukulam':
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                print(request.POST.get('cropped_image'))
                '''
...
def event_list(request):
    ...
    locations = Location.objects.all()
    user_counts = User.objects.filter(groups__name__in=('Musician','Music School','Event Organizer','Band')).values('groups__name').annotate(total=Count('groups__name')).order_by('total')
    ...
    for user_count in user_counts:
        ...
        elif user_count['groups__name'] == "Musician":
            musician_count = user_count['total']

    
    popular_musicians = User.objects.filter(groups__name="Musician").annotate(numof_participating_event_count=Count('musician__minvitation')+Count('musician__minterest')).order_by('-numof_participating_event_count')[:5]
...
def view_musicians(request):
    ...
    if location_id and  location_id != "Select Location":
        users  = User.objects.filter(groups__name="Musician",location__id=location_id)
    else:
        users  = User.objects.filter(groups__name="Musician")
    ...
    def get(self, request):
        ...
        for group in user_groups:
            if group.name == "Musician":
                ...
                event_performer_serializer = EventPerformerSerializer(event_performer_requests, many=True)
                ...
                context = {
                    ...
                    'event_performer_requests': event_performer_serializer.data,
                ...
                event_profile_Serializer = Event_profileSerializer(request.user)
                event_aboutme_Serializer = Event_aboutmeSerializer(event_about_me)
                event_performer_Serializer = Event_Performer_Serializer(event_performers,many=True)
                event_highlights_Serializer = Event_Highlight_Serializer(event_highlights,many=True)
                context = {
                    'user' : event_profile_Serializer.data,
                    'event_about_me': event_aboutme_Serializer.data,
                ...
                troupe_profile_Serializer = Troupe_profileSerializer(request.user)
                ...
                musicschool_profile_Serializer = Musicschool_profileSerializer(request.user)
                ...
                musicschool_award_Serializer = Musicschool_Award_Serializer(musicschool_awards,many=True)
...
class Musician_Edit_Api(APIView):
    ...
    def put(self, request, format=None):
        user  = User.objects.get(id=request.user.id)
        musician = Musician.objects.get(user=request.user)
        user_serializer = User_edit_Serializer(user, data=request.data)
        musician_serializer = Musician_edit_Serializer(musician, data=request.data)
...
class Event_Edit_Api(APIView):
    ...
    def put(self, request, format=None):
        ...
        event = Event_Organizer.objects.get(user=request.user)
        user_serializer = User_edit_Serializer(user, data=request.data)
        event_serializer = Event_edit_Serializer(event, data=request.data)
        if event_serializer.is_valid() and user_serializer.is_valid():
            ...
            return Response(event_serializer.errors,user_serializer.errors)
    ...
    def get(self, request, pk):
        ...
        for group in user_groups:
            if group.name == "Musician":
                ...
                event_performer_requests = Event_Performer.objects.filter(musician=user.musician)
                ...
                context = {
                    'user' : my_profile_Serializer.data,
                    'about_me' : aboutme_Serializer.data,
                    'skills' : myskills_Serializer.data,
                    'highlights' : highlights_Serializer.data,
                    'awards' : award_Serializer.data,
                ...
                event_profile_Serializer = Event_profileSerializer(user)
                event_aboutme_Serializer = Event_aboutmeSerializer(event_about_me)
                event_performer_Serializer = Event_Performer_Serializer(event_performers,many=True)
                event_highlights_Serializer = Event_Highlight_Serializer(event_highlights,many=True)
                event_post_Serializer = Event_Post_Serializer(event_posts,many=True)
                context = {
                    'event_posts': event_post_Serializer.data,
                    'user' : event_profile_Serializer.data,
                    'event_about_me': event_aboutme_Serializer.data,
                    'event_performers': event_performer_Serializer.data,
                    'highlights': event_highlights_Serializer.data,
                ...
                troupe_profile_Serializer = Troupe_profileSerializer(user)
                troupe_aboutme_Serializer = Troupe_aboutmeSerializer(troupe_about_me)
                troupe_key_performer_Serializer = Troupe_Key_Performer_Serializer(troupe_key_performer,many=True)
                troupe_skills_Serializer= Troupe_Skill_Serializer(troupe_skills ,many=True)
                troupe_highlights_Serializer= Troupe_Highlight_Serializer(troupe_highlights,many=True)
                ...
                musicschool_profile_Serializer = Musicschool_profileSerializer(user)
                musicschool_aboutme_Serializer = Musicschool_aboutmeSerializer(musicschool_about_me )
                musicschool_skills_Serializer= Musicschool_Skill_Serializer(musicschool_skills ,many=True)
                musicschool_award_Serializer = Musicschool_Award_Serializer(musicschool_awards,many=True)
                musicschool_highlights_Serializer= Musicschool_Highlight_Serializer(musicschool_highlights,many=True)
...
class Following_API(APIView):
    def get(self, request):
        followings = UserFollowing.objects.filter(user=request.user)
        serilaizer  = Following_Serializer(followings, many=True)
...
class All_People_API(APIView):
    def get(self, request):
        users = User.objects.filter(is_active=True).exclude(Q(id=request.user.id) | Q(is_superuser=True))
        serilaizer  = All_People_Serializer(users, many=True)
        return Response(serilaizer.data)
